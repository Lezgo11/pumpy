...
# ------------------------
# Very simple volume integrator for PV logging
# ------------------------
class VentricularVolumeLogger: # ventricular
    """Integrates LV cavity volume from inflow/outflow when you have valve states.
    dV/dt = Q_in - Q_out, with simple valve resistances.
    This is *not* fluid; it is a bookkeeping model that yields a usable PV loop.
    Provide pressures in Pa and valve booleans each step.
    """
    ...
    def step(self, t, P_LA, P_LV, P_AO, mitral_open, aortic_open, dt):
        ...
        # log PV using *effective* LV pressure as y-axis
        self.log["t"].append(t)
    ...
    def export_csv(self, path="pv_lv.csv"):
        ...
        return path
class AtrialVolumeLogger: # atrial 
    """Simple LA bookkeeping: dV/dt = Q_pv_in - Q_mitral_out.
    Q_pv_in flows when P_PV > P_LA (no valve); Q_mitral_out when mitral is open.
    """
...
def make_solid(domain, facet_tags, load_tags, bc_tags, E=10e3, nu=0.49, traction_dir=(0.0, 1.0, 0.0)):
    ...
    # boundary conditions (pin a few dofs)
    ...
    def apply_and_solve(p_scalar):
        ...
        for tag in load_tags:
            ...
            #L += ufl.dot(t_vec, ufl.TestFunction(V)) * ds(tag)
    ...
    return {"domain": domain, "V": V, "u": u, "F_res": F_res, "J": Jform, "ds": ds,
            "facet_tags": facet_tags, "apply_and_solve": apply_and_solve}


# ------------------------
# A tiny Aortic Windkessel-2 (for afterload) and valve logic
# ------------------------
...
# ------------------------
# Main (supports separate meshes: atrium.msh + ventricle.msh)
# ------------------------
if __name__ == "__main__":
    time_start = time.time()
    # ---- CONFIG: point to your separate mesh files ----
    ATRIUM_MSH = "mesh/hollow_sphere_LA.msh"       # LA mesh file
    VENTRICLE_MSH = "mesh/idealized_LV.msh" # LV mesh file idealized_LV
    ATRIUM_MSH_ADV = "mesh/LA_adv.msh"


    # ---- Read meshes independently (only those you activate) ----
    LA = None
    LV = None

    if "LA" in ACTIVE_CHAMBERS:
        msh_A, ct_A, ft_A = gmshio.read_from_msh(ATRIUM_MSH, MPI.COMM_WORLD, 0, gdim=3)
        LA_adv = False
        LA = make_solid(msh_A, ft_A, load_tags=(10,), bc_tags=20)

    if "LV" in ACTIVE_CHAMBERS:
        msh_V, ct_V, ft_V = gmshio.read_from_msh(VENTRICLE_MSH, MPI.COMM_WORLD, 0, gdim=3)
        LA_adv = False
        LV = make_solid(msh_V, ft_V, load_tags=(), bc_tags=None)  #originally 20 and 40 
    
    if "LA_adv" in ACTIVE_CHAMBERS:
        msh_A, ct_A, ft_A = gmshio.read_from_msh(ATRIUM_MSH_ADV, MPI.COMM_WORLD, 0, gdim=3)
        LA_adv = True
    ...
    log = {"t": [], "P_PV(Pa)": [], "P_LA(Pa)": [], "P_LV_raw(Pa)": [], "P_LV_eff(Pa)": [],
       "mitral_open": [], "aortic_open": []}

    # simple elastance function to generate LV raw pressure from activation (optional)
    def elastance(t):
        # basic time-varying elastance normalized to [0,1]
        phi = (t % T_cyc) / T_cyc
        return 0.05 + 0.95 * smooth_cos_window((phi + 0.1) % 1.0)

    # placeholder LV passive pressure baseline
    P_LV_passive = 8.0 * MMHG_TO_PA

    t = 0.0
    while t <= T_end + 1e-12:
        # --- Electrophysiology / activation -> active stress scalar (kPa)
        active_stress_kPa = 0.0
        if ep:
            a = ep.step()
            active_stress_kPa = ep.active_stress_kPa()
            # TODO: wire active_stress_kPa into your LV constitutive law via a coefficient Function when ready.
        ...
        # --- Atrial pressure (exists only if LA active)
        if "LA" in ACTIVE_CHAMBERS:
            P_LA = atrial_pressure_pa(t, T_cyc)
            P_PV = pv_src.pressure(t) if pv_src else 12.0 * MMHG_TO_PA
            ...
            P_LA = 10.0 * MMHG_TO_PA  # TODO: Make a better default (function)
        # --- LV pressures & valves (only if LV active)
        if "LV" in ACTIVE_CHAMBERS:
            P_AO = ao.P if ao is not None else  30* MMHG_TO_PA # TODO: Make a better default (function)
            P_LV_raw = P_LV_passive + 60e3 * elastance(t)
            mitral_open = P_LA > P_LV_raw
            aortic_open = P_LV_raw > P_AO

            # effective LV pressure applied to LV solid
            if aortic_open:
                P_LV_eff = P_LV_raw
            elif mitral_open:
                P_LV_eff = P_LA
        ...
        # --- Apply loads & solve (on each separate solid)
        if LA is not None:
        ...
        if MPI.COMM_WORLD.rank == 0:
            print(f"Simulation completed in {its_la if LA else its_lv}[t={t:.3f}] LA={'on' if LA else 'off'} LV={'on' if LV else 'off'} | "
        ...
        # --- Volume logging & afterload updates
        
        if LV is not None:
            if vlogger is not None:
                vlogger.step(t, P_LA, P_LV_eff if np.isfinite(P_LV_eff) else P_LA, P_AO if np.isfinite(P_AO) else P_LA, mitral_open, aortic_open, dt)
    ...
    if MPI.COMM_WORLD.rank == 0:
        ...
        if vlogger:
           print("Wrote", vlogger.export_csv("pv_lv.csv"))
        if alogger:
            print("Wrote", alogger.export_csv("pv_la.csv"))
        # LV PV plot
        if vlogger:
        ...
        if np.isfinite(np.nanmean(log["P_LV_raw(Pa)"])):
            ...
            plt.plot(t_arr, np.array(log["P_LV_eff(Pa)"])*PA_TO_MMHG, label="P_LV_eff", ls="--")
            plt.step(t_arr, np.array(log["mitral_open"]) * 20, where="post", label="mitral (×20)")
            plt.step(t_arr, np.array(log["aortic_open"]) * 20, where="post", label="aortic (×20)")
