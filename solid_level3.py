...
from read_fileh5 import read_cardiac_atlas, create_mesh_from_points  # Assuming this function is defined in read_fileh5.py
# Time parameters
T = 1.0            # final time
...
h5_path = "UKBRVLV.h5"
# 1. Read the H5 file
ed, es = read_cardiac_atlas(h5_path)
# 2. Create and save ED mesh
comm = MPI.COMM_WORLD
domain = create_mesh_from_points(ed)
# Create unit cube mesh with hexahedral elements
# Define the element type and function space
element = basix.ufl.element(
    family="Lagrange",  # or "CG"
    cell="triangle",  # since your mesh is hexahedral, hexahedron
    degree=1,
...
plotter.open_gif("deformation_heart.gif", fps=5)
...
if domain.comm.rank == 0:
    ...
    plt.grid(True)
    plt.savefig("displacement_heart.png")
    plt.show()